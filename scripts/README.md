...
| `assets/` | Documented repairs of question pictures (`fix_*.py`, report via `build_asset_fix_report.py` → `data/asset-fix-report.json`), and `crop_book_figures.py`: crops the textbook figures that replace small or blurry exam pictures (`data/book-figure-crops.json` → `assets/questions/q-book-*.png` + `data/book-figures.js`). | Yes: page renders and originals in `tmp/`; the textbook PDF |
